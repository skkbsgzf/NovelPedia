...
产出(均在 {--out}):
  ...
  - knowledge_graph.html : 数据内嵌的自包含单文件浏览器直接打开, 无需服务器
...
def main():
    ...
    OUT_HTML = os.path.join(OUT_DIR, "knowledge_graph.html")
    ...
    setting_hits = defaultdict(set)       # 设定名-> 出现章节(文本命中)
    ...
    for ch, who_j, act_j, notes, where in rows:
        ...
        # 场景文本: 用于设定命中
        blob = " ".join(filter(None, [act_j, notes or "", where or ""]))
        if blob:
            for s in sets_:
                nm = str(s.get("name", "")).strip()
                if nm and len(nm) >= 2 and nm in blob:
                    setting_hits[nm].add(ch)
    ...
    for s in sets_:
        ...
        if canon not in merged:
            merged[canon] = {"type": s.get("type", ""), "desc": s.get("description", ""),
                             ...
                             "chapters": sorted(setting_hits.get(name, set()))}
            order.append(canon)
        g = merged[canon]
        if s.get("first_seen"):
    ...
    for a, d in cooccur.items():
        for b, cnt in d.items():
            if cnt < 2:
                continue
            key = frozenset((a, b))
            if any(e["kind"] == "relation" and frozenset((e["from"][2:], e["to"][2:])) == key for e in edges):
                continue
            edges.append({
                "from": "c:" + a, "to": "c:" + b,
    ...
    for name in order:
        ...
        for r in g["related"]:
            ...
            chs = sorted(a_chs | set(setting_hits.get(b, set())))
            edges.append({
                "from": "s:" + name, "to": b_id,
                "kind": "related", "label": "关联",
                "chapters": chs,
            })
        # 4d. 代号归属: "X在Y中的代号" 应连回真实角色(如 正义 -> 奥黛丽)
        owner = detect_codename_owner(g["desc"], set(char_by_main))
        if owner:
            key = frozenset(("s:" + name, "c:" + owner))
            if key not in seen_related:
                seen_related.add(key)
                edges.append({
                    "from": "s:" + name, "to": "c:" + owner,
                    "kind": "related", "label": "代号归属",
                    "chapters": sorted(a_chs),
                })
    ...
    if os.path.exists(TEMPLATE):
        ...
        js = js.replace("</", "<\\/")   # 防止 </script> 提前闭合
        html = tpl.replace("__GRAPH_DATA__", js)
        with open(OUT_HTML, "w", encoding="utf-8") as f:
            f.write(html)
        print(f"已生成{OUT_HTML} (自包含单文件)")
    else:
        print(f"[warn] 模板缺失: {TEMPLATE}, 仅输出JSON")
